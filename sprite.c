...
void		calc_sprite(t_parser *map)
{
	...
	while (tmp != NULL)
	{
		tmp->sprite_dir = atan2(map->y1 - tmp->sprite_y,
						tmp->sprite_x - map->x1);
		if (tmp->sprite_dir < 0 && map->p_angle >= M_PI / 6
		&& map->p_angle <= (M_PI * 2 - M_PI / 6))
		...
		if (tmp->sprite_dir > -M_PI / 6 && tmp->sprite_dir < M_PI / 6)
			tmp->sprite_dist = sqrt(pow(map->x1 - tmp->sprite_x, 2) +
					pow(map->y1 - tmp->sprite_y, 2));
		else
			tmp->sprite_dist = -1;
...
void		sort_sprite(t_parser *map, t_sprite *tmp, t_sprite *tmp_max)
{
	...
	while (map->max != -1)
	{
		...
		while (tmp != NULL)
		{
			if (tmp->sprite_dist > map->max)
			{
				map->max = tmp->sprite_dist;
				tmp_max = tmp;
			}
			tmp = tmp->next;
		}
...
int			draw_sprite(t_parser *map, t_sprite *tmp)
{
	...
	tmp->x = map->width / 2 - (map->width / (M_PI / 3))
			* tmp->sprite_dir - tmp->h / 2;
	...
	while (++x < tmp->h && (x + tmp->x) < map->width)
	{
		...
		{
			color = map->tex4.t_addr[(int)(y / tmp->size) *
							map->tex4.w + (int)(x / tmp->size) % map->tex4.h];
			if (color > 0 && map->dist_wall[(int)(tmp->x + x)]
			> tmp->sprite_dist)
